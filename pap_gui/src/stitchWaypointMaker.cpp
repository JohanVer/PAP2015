...
std::vector<QVector3D> generateWaypoints(const QVector3D &init_point, unsigned int min_p_overlap, const QVector2D &pcb_size, double px_conv, double py_conv, double camera_pcb_height){
    const unsigned int px_fov = 480;
    const unsigned int py_fov = 640;
    ...
    // Generate waypoints
    std::cerr << "Generating waypoints... x im: " << images_x << " y_im: " << images_y << std::endl;
    for(size_t n_x = 0; n_x < images_x; n_x++){
        for(size_t n_y = 0; n_y < images_y; n_y++){
            ...
            QVector3D p = init_point - QVector3D(x_pos, y_pos, 0);
            waypoints.push_back(p);
        }
        if(n_x + 1 < images_x){
            n_x++;
            for(int n_y = images_y-1; n_y >= 0;  n_y = n_y -1){
