...
motorController::motorController() {
    ...
    serialPort_ = std::unique_ptr<serial::Serial> (new serial::Serial("/dev/ttyUSB0", 38400,
...
void motorController::ReqParker(unsigned char rw, unsigned char* data,
    ...
    size_t bytes_wrote = serialPort_->write((uint8_t*) dataComplete,
                                          (size_t) totalLength);
...
unsigned int motorController::readInt16AddressParker(
    ...
    int readBytes = serialPort_->read(readBuffer, numToRead);
...
bool motorController::checkForACK() {
    ...
    int readBytes = serialPort_->read(readBuffer, totalLength);
...
void motorController::connectToBus() {
    if (serialPort_->isOpen())
        ROS_INFO("serialPort_ opened...");
    else
        ROS_ERROR("serialPort_ could not be opened");
}
...
bool motorController::isConnected(enum pap_common::MOTOR device){
    switch(device){
    case pap_common::XMOTOR:
        return controllerConnected_1_;
        break;

    case pap_common::YMOTOR:
        return controllerConnected_2_;
        break;

    case pap_common::ZMOTOR:
        return controllerConnected_3_;
        break;
    }
}

bool motorController::disconnect(enum pap_common::MOTOR device){
    switch(device){
    case pap_common::XMOTOR:
        controllerConnected_1_ = false;
        break;
...
controllerStatus motorController::getFullStatusController(enum pap_common::MOTOR addressDevice){
    controllerStatus controllerStatusAct = getStatusController((unsigned char)addressDevice);
    controllerStatusAct.position = getPosition((unsigned char)addressDevice);
    return controllerStatusAct;
}

bool motorController::energizeAxis(enum pap_common::MOTOR adressDevice, bool trigger) {
    if (adressDevice == pap_common::XMOTOR) {
        if (!controllerConnected_1_) {
            ROS_ERROR("Controller 1 not connected");
            return false;
        }
    }
    if (adressDevice == pap_common::YMOTOR) {
        if (!controllerConnected_2_) {
            ROS_ERROR("Controller 2 not connected");
            return false;
        }
    }
    if (adressDevice == pap_common::ZMOTOR) {
...
bool motorController::manual(enum pap_common::MOTOR deviceAddress,
...
bool motorController::stop(enum pap_common::MOTOR deviceAddress) {
